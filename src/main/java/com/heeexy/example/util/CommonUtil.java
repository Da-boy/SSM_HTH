...
public class CommonUtil {
	...
	public static JSONObject successPage1(List<JSONObject> list, int totalCount) {
		JSONObject result = successJson();
		JSONObject info = new JSONObject();
		info.put("list", list);
		info.put("totalCount", totalCount);
...
//				throw new CommonJsonException(jsonObject);
				jsonObject.clear();
				jsonObject.put("code",ErrorEnum.E_90003.getErrorCode());
				jsonObject.put("msg","请填写完整后再点击创建！");
				throw  new CommonJsonException(jsonObject);
			}
		}
	}
	...
	/**
	 * 取系统时间，创建格式化时间标签，保存数据库表用
	 */
	public static String getCurTimestampForDB() {
		//使用Date创建日期对象
		Date date = new Date();
		System.out.println("当前的日期是------>"+date);
