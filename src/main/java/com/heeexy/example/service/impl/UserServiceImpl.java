...
import com.heeexy.example.dao.UserPositionDao;
...
public class UserServiceImpl implements UserService {
	...
	@Override
	public JSONObject listUserBySearch(JSONObject jsonObject) {
		CommonUtil.fillPageParam(jsonObject);
		int count = userDao.countUserBySearch(jsonObject);
		System.out.println("count:"+count);
		List<JSONObject> list = userDao.listUserBySearch(jsonObject);
		return CommonUtil.successPage(jsonObject, list, count);
	}
	...
	public JSONObject addUser(JSONObject jsonObject) {
		...
		JSONObject usernameAndEmail = new JSONObject();
		usernameAndEmail.put("username",jsonObject.getString("username"));
		usernameAndEmail.put("email",jsonObject.getString("email"));
		List<JSONObject> newUser = userDao.getUserByusernameAndEmail(usernameAndEmail);
		System.out.println("newUserSIZE:"+newUser.size());
		System.out.println("userID---new:"+newUser.get(0).get("id"));
		jsonObject.put("userId",newUser.get(0).get("id"));
		jsonObject.put("global_size","medium");
		jsonObject.put("global_color_theme","#11A983");
		jsonObject.put("sale_print_favourite","deliver");
		System.out.println("jsonObject:::::::::::::::::"+jsonObject);
		settingDao.addGlobalConfig(jsonObject);
		personnelDao.addPersonnel(jsonObject);
		userPositionDao.addUserPosition(jsonObject);
	...
	public JSONObject updateUser(JSONObject jsonObject) {
		System.out.println("jjjjjjjjjjjjj:"+jsonObject);
		int deleteStatus = jsonObject.getIntValue("deleteStatus");
		if(deleteStatus == 2){
			userDao.updateUser(jsonObject);
		}else{
			userDao.updateUser(jsonObject);
			userPositionDao.updateUserPosition(jsonObject);
		}
		return CommonUtil.successJson();
	}
	...
	@Override
	public JSONObject getAllProjects(JSONObject jsonObject) {
		List<JSONObject> list = userDao.getAllProjects(jsonObject);
		return CommonUtil.successPage(list);
	}

	@Override
	public JSONObject getAllProjectsByPerson(JSONObject jsonObject) {
		List<JSONObject> list = userDao.getAllProjectsByPerson(jsonObject);
		return CommonUtil.successPage(list);
	}
}
